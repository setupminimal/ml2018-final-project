...
theme_set(theme_grey() + theme(legend.justification = "left", 
                               plot.margin = margin(0, 0, 0, 0)))

spDES <- ggplot(DES, aes(x = Time, y = value, color = variable)) + 
  geom_smooth(method = "lm", formula = y ~ ns(x, df = free), se = FALSE) + 
  remove_axis + ylab("DES") + guides(color = guide_legend(title = "DES")) + 
  labs(title = "Features Grouped by Instrument and Time")
spDIS <- ggplot(DIS, aes(x = Time, y = value, color = variable)) + 
  geom_smooth(method = "lm", formula = y ~ ns(x, df = free), se = FALSE) + 
  remove_axis + ylab("DIS") + guides(color = guide_legend(title = "DIS")) + 
  theme(plot.title = element_blank())
spFGM <- ggplot(FGM, aes(x = Time, y = value, color = variable)) + 
  geom_smooth(method = "lm", formula = y ~ ns(x, df = free), se = FALSE) + 
  remove_axis + ylab("FGM") + guides(color = guide_legend(title = "FGM"))
pSEL <- ggplot(SEL, aes(x = X, y = Y)) + geom_line() + xlab("Time in Seconds") +
...
cowplot::plot_grid(
  cowplot::plot_grid(spDES + remove_legend, spDIS + remove_legend, spFGM + 
                     remove_legend, pSEL, align = "hv", ncol = 1, 
                     rel_heights = c(1, 1, 1, 0.4)),
  cowplot::plot_grid(lDES, lDIS, lFGM, NULL, ncol = 1, 
                     rel_heights = c(1, 1, 1, 0.4)),
  rel_widths = c(5, 1)
)
```
...
The time series hypothesis I have is that data point n is dependent on the 
previous data (n-1, n-2, etc.): it's not independant of the past. Therefore, 
when we consider this model we should look at how data point n differs from 
that of the past.

My time series function looks at m number of past data points and takes the
average difference. For basic comparison below, I've chosen m = 20 and this 
seems to be a good generalization without loosing the scope of what we could 
consider 'recent past'. Using m = 20 is around 1.5 minutes of low resolution 
data. Larger m values have diminishing returns on their ability to reduce error, 
and coulld possibly lead to overfiting.

First we run the data on a controll model, in this case Logisitc regression 
with Lasso, using both the raw data and then the 'time seires' data that 
contains columns of past average difference. Then we do the same with our 
selected model(s), Radial SVM, etc.

We compare, using the evaluation method documented in the evaluation file, by 
finding the number of found and missed SITL selections, as well as the 
missclassification error and performance error(Priority^2 of missed SITL 
selections.
...
log.lasso.fit <- glmnetUtils::cv.glmnet(Selected ~ . - Time - Priority - SelectedF, 
...
diffData <- data.frame(X = data$X, series_metric(data[,featureSubset], 20), 
                       data[, SEL_set], Priority = data$Priority, 
                       SelectedF = data$SelectedF)
...
Running crossvalidation to pick the optimim m value and model type might 
overfit, so we could use the second dataset as a validation set to calculate 
true performance.
