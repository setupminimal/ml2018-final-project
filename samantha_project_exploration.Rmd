...
```{r ts_base_comparison}
train <- split_train(data, 100)
...
cat("Basic Lasso Logistic Regression Error\n")
#f = "basic.csv"
#save_predictions(probabilities, data$X, f)
#evaluate_file(data, f)
evaluate_data(data, probabilities)
```
```{r ts_series_build}
diffData <- data.frame(X = data$X, series_metric(data[,featureSubset], 20), data[, SEL_set], Priority = data$Priority)
diffTrain <- split_train(diffData, 100)
diffTest <- split_test(diffData, 100)
```
```{r ts_series_testdata}
log.lasso.fit <- glmnetUtils::cv.glmnet(Selected ~ . - Time - Priority, data = diffTrain, family = "binomial")
log.odds <- predict(log.lasso.fit, diffTest)
probabilities <- exp(log.odds) / (1 + exp(log.odds))
cat("Time Series Test error\n")
evaluate_data(diffTest, probabilities)
```
```{r ts_series_wholedata}
log.lasso.fit <- glmnetUtils::cv.glmnet(Selected ~ . - Time - Priority, data = diffTrain, family = "binomial")
log.odds <- predict(log.lasso.fit, diffData)
probabilities <- exp(log.odds) / (1 + exp(log.odds))
cat("Time Series Total Error\n")
#f = "time.series.metric.csv"
#save_predictions(probabilities, diffData$X, f)
#evaluate_file(diffData, f)
