...
series <- function(x) {
  scols <- function(x, pre) {
    ...
    colnames(tmp) <- c(paste(pre, "1"), paste(pre, "2"), paste(pre, "3"), paste(pre, "4"), paste(pre, "5"))
    tmp
  }
  names <- colnames(x)
...
probs <- predict(log, test[,c(1:6, 13:15, 18:19)], type = "response")
plot(log)

timeseries <- glm(Selected ~ ., data = data.frame(train[3:(dim(train)[1] - 2), 18:19], series(train[,c(1:6, 13:15)])), family = binomial)
plot(timeseries)
summary(timeseries)
```