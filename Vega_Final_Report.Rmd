...
row.names(rf.diff.eval) <- c("Difference Test", "Difference Train", "Difference Total")
```
...
```
```{r}
pred <- predict(rf.shift.fit, newdata = shift.test)
eval.test <- evaluate_data_asFrame(shift.test, pred)

pred <- predict(rf.shift.fit, newdata = shift.train)
eval.train <- evaluate_data_asFrame(shift.train, pred)

pred <- predict(rf.shift.fit, newdata = shift.total)
eval.total <- evaluate_data_asFrame(shift.total, pred)

rf.shift.eval <- rbind(eval.test, eval.train, eval.total)
row.names(rf.shift.eval) <- c("Shift Test", "Shift Train", "Shift Total")
```
...
```
```{r}
pred <- predict(rf.base.fit, newdata = base.test)
eval.test <- evaluate_data_asFrame(base.test, pred)

pred <- predict(rf.base.fit, newdata = base.train)
eval.train <- evaluate_data_asFrame(base.train, pred)

pred <- predict(rf.base.fit, newdata = base.total)
eval.total <- evaluate_data_asFrame(base.total, pred)

rf.base.eval <- rbind(eval.test, eval.train, eval.total)
row.names(rf.base.eval) <- c("Base Test", "Base Train", "Base Total")
```
