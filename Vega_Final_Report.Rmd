...
## Linear Regression
```{r lin_eval_function}
elin <- function(fit, tr, tst, prefix = ""){
  ...
  evals <- rbind(eval.test, eval.train)
...
lin.diff.fit <- lm(Priority ~ . - X, data = subset)
subset <- shift.train[ ,!(colnames(shift.train) %in% c("Selected"))]
lin.shift.fit <- lm(Priority ~ . - X, data = subset)
subset <- base.train[ ,!(colnames(base.train) %in% c("Selected"))]
lin.base.fit <- lm(Priority ~ . - X, data = subset)
```
```{r lin_evals}
lin.eval <- rbind(elin(lin.base.fit, base.train, base.test, "Base"),
                   elin(lin.shift.fit, shift.train, shift.test, "Shift"),
                   elin(lin.diff.fit, diff.train, diff.test, "Difference"))
kable(lin.eval, "html") %>% kable_styling(bootstrap_options = "condensed") %>% 
  column_spec(5:6, bold = T) %>%
  group_rows("Basic Data", 1, 2) %>%
  group_rows("Time Shift Data", 3, 4) %>%
  group_rows("Difference Data", 5, 6)
```

## Radial SVM
```{r rsvm_eval_function}
esvm <- function(fit, tr, tst, prefix = ""){
  pred <- predict(fit, tst)
  eval.test <- evaluate_data_asFrame(tst, pred)
...
```{r rsvm_models}
subset <- diff.train[ ,!(colnames(diff.train) %in% c("Selected"))]
rsvm.diff.fit <- svm(Priority ~ . - X, data = subset, kernel = "radial")
subset <- na.omit(shift.train[ ,!(colnames(shift.train) %in% c("Selected"))])
rsvm.shift.fit <- svm(Priority ~ . - X, data = subset, kernel = "radial")
subset <- base.train[ ,!(colnames(base.train) %in% c("Selected"))]
rsvm.base.fit <- svm(Priority ~ . - X, data = subset, kernel = "radial")
```
```{r rsvm_evals}
rsvm.eval <- rbind(esvm(rsvm.base.fit, base.train, base.test, "Base"),
                   esvm(rsvm.shift.fit, shift.train, shift.test, "Shift"),
                   esvm(rsvm.diff.fit, diff.train, diff.test, "Difference"))
kable(rsvm.eval, "html") %>% kable_styling(bootstrap_options = "condensed") %>% 
  column_spec(5:6, bold = T) %>%
  group_rows("Basic Data", 1, 2) %>%
  group_rows("Time Shift Data", 3, 4) %>%
  group_rows("Difference Data", 5, 6)
...
treeCount = 500
```
Using `r depth ` splits and `r treeCount ` trees.

```{r rf_eval_function}
etrees <- function(fit, tr, tst, prefix = ""){
...
```
```{r rf_evals}
rf.eval <- rbind(etrees(rf.base.fit, base.train, base.test, "Base"),
                 etrees(rf.shift.fit, shift.train, shift.test, "Shift"),
                 etrees(rf.diff.fit, diff.train, diff.test, "Difference"))
kable(rf.eval, "html") %>% kable_styling(bootstrap_options = "condensed") %>% 
  column_spec(5:6, bold = T) %>%
  group_rows("Basic Data", 1, 2) %>%
  group_rows("Time Shift Data", 3, 4) %>%
  group_rows("Difference Data", 5, 6)
...
row.names(rf.diff.val) <- c("Validation")

kable(rf.diff.val, "html") %>% kable_styling(bootstrap_options = "condensed") %>% 
  column_spec(4:6, bold = T)
```
